namespace NovaEngine.ContentPipeline.Font.EdgeSegments;
...
internal abstract class EdgeSegmentBase
{
    ...
    *********/
    ...
    public abstract Vector2<float> Direction(float amount);
    ...
    public abstract Vector2<float> Lerp(float amount);

    /// <summary>Gets the signed distance from a point to the segment.</summary>
    /// <param name="point">The point to get the signed distance of.</param>
    /// <param name="t">
    ...
    public void DistanceToPseudoDistance(SignedDistance distance, Vector2<float> point, float t)
    {
        if (t < 0)
        {
            ...
            var ts = Vector2<float>.Dot(apDirection, direction);
            if (ts < 0)
            {
        ...
        else if (t > 1)
        {
            ...
            var ts = Vector2<float>.Dot(bpDirection, direction);
            if (ts > 0)
            {
