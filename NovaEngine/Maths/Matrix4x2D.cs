...
public struct Matrix4x2D : IEquatable<Matrix4x2D>
{
    ...
    *********/
    ...
    public readonly Matrix2x4D Transposed => new(M11, M21, M31, M41, M12, M22, M32, M42);
    ...
    public Vector2<double> Row4
    {
        readonly get => new(M41, M42);
        set
        {
            M41 = value.X;
            M42 = value.Y;
    ...
    *********/
    ...
    public Matrix4x2D(in Vector2<double> row1, in Vector2<double> row2, in Vector2<double> row3, in Vector2<double> row4)
    {
        M11 = row1.X;
        M12 = row1.Y;
        M21 = row2.X;
    ...
    /// <param name="m41">The first element of the fourth row.</param>
    /// <param name="m42">The second element of the fourth row.</param>
    public Matrix4x2D(in Matrix2x2D matrix, double m31 = 0, double m32 = 0, double m41 = 0, double m42 = 0)
    {
        M11 = matrix.M11;
        M12 = matrix.M12;
        M21 = matrix.M21;
    ...
    /// <param name="m41">The first element of the fourth row.</param>
    /// <param name="m42">The second element of the fourth row.</param>
    public Matrix4x2D(in Matrix3x2D matrix, double m41 = 0, double m42 = 0)
    {
        M11 = matrix.M11;
        M12 = matrix.M12;
        M21 = matrix.M21;
    ...
    public readonly Matrix4x2 ToMatrix4x2() => new((float)M11, (float)M12, (float)M21, (float)M22, (float)M31, (float)M32, (float)M41, (float)M42);

    /// <inheritdoc/>
    public readonly bool Equals(Matrix4x2D other) => this == other;

    /// <inheritdoc/>
    public readonly override bool Equals(object? obj) => obj is Matrix4x2D matrix && this == matrix;
    ...
    public static Matrix4x2D CreateRotation(double angle) => new(Matrix2x2D.CreateRotation(angle));

    /// <summary>Creates a scale matrix.</summary>
    /// <param name="scale">The uniform scale factor.</param>
    /// <returns>The created matrix.</returns>
    public static Matrix4x2D CreateScale(double scale) => new(Matrix2x2D.CreateScale(scale));

    /// <summary>Creates a scale matrix.</summary>
    /// <param name="xScale">The scale factor of the X axis.</param>
    /// <param name="yScale">The scale factor of the Y axis.</param>
    /// <returns>The created matrix.</returns>
    public static Matrix4x2D CreateScale(double xScale, double yScale) => new(Matrix2x2D.CreateScale(xScale, yScale));

    /// <summary>Creates a scale matrix.</summary>
    /// <param name="scale">The scale factor of the X and Y axis.</param>
    /// <returns>The created matrix.</returns>
    public static Matrix4x2D CreateScale(Vector2<double> scale) => new(Matrix2x2D.CreateScale(scale));
    ...
    *********/
    ...
    public static Matrix4x2D operator *(Matrix4x2D left, Matrix2x2D right)
    {
        return new(
            ...
            m31: Vector2<double>.Dot(left.Row3, right.Column1),
            m32: Vector2<double>.Dot(left.Row3, right.Column2),
            m41: Vector2<double>.Dot(left.Row4, right.Column1),
            m42: Vector2<double>.Dot(left.Row4, right.Column2)
        );
    }
    ...
    public static Matrix4x3D operator *(Matrix4x2D left, Matrix2x3D right)
    {
        return new(
            ...
            m31: Vector2<double>.Dot(left.Row3, right.Column1),
            m32: Vector2<double>.Dot(left.Row3, right.Column2),
            m33: Vector2<double>.Dot(left.Row3, right.Column3),
            m41: Vector2<double>.Dot(left.Row4, right.Column1),
            m42: Vector2<double>.Dot(left.Row4, right.Column2),
            m43: Vector2<double>.Dot(left.Row4, right.Column3)
        );
    }
    ...
    public static Matrix4x4D operator *(Matrix4x2D left, Matrix2x4D right)
    {
        return new(
            ...
            m32: Vector2<double>.Dot(left.Row3, right.Column2),
            m33: Vector2<double>.Dot(left.Row3, right.Column3),
            m34: Vector2<double>.Dot(left.Row3, right.Column4),
            m41: Vector2<double>.Dot(left.Row4, right.Column1),
            m42: Vector2<double>.Dot(left.Row4, right.Column2),
            m43: Vector2<double>.Dot(left.Row4, right.Column3),
            m44: Vector2<double>.Dot(left.Row4, right.Column4)
        );
    }
