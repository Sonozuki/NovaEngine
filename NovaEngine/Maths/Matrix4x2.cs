...
/// <summary>Represents a 4x2 matrix using single-precision floating-point numbers.</summary>
public struct Matrix4x2 : IEquatable<Matrix4x2>
{
    ...
    *********/
    ...
    public readonly Matrix2x4 Transposed => new(M11, M21, M31, M41, M12, M22, M32, M42);
    ...
    *********/
    ...
    public Matrix4x2(float m11, float m12, float m21, float m22, float m31, float m32, float m41, float m42)
    {
        M11 = m11;
        M12 = m12;
        M21 = m21;
    ...
    public Matrix4x2(in Vector2<float> row1, in Vector2<float> row2, in Vector2<float> row3, in Vector2<float> row4)
    {
        M11 = row1.X;
        M12 = row1.Y;
        M21 = row2.X;
    ...
    /// <param name="m41">The first element of the fourth row.</param>
    /// <param name="m42">The second element of the fourth row.</param>
    public Matrix4x2(in Matrix2x2 matrix, float m31 = 0, float m32 = 0, float m41 = 0, float m42 = 0)
    {
        M11 = matrix.M11;
        M12 = matrix.M12;
        M21 = matrix.M21;
    ...
    /// <param name="m41">The first element of the fourth row.</param>
    /// <param name="m42">The second element of the fourth row.</param>
    public Matrix4x2(in Matrix3x2 matrix, float m41 = 0, float m42 = 0)
    {
        M11 = matrix.M11;
        M12 = matrix.M12;
        M21 = matrix.M21;
    ...
    /// <summary>Gets the matrix as a <see cref="Matrix4x2D"/>.</summary>
    /// <returns>The matrix as a <see cref="Matrix4x2D"/>.</returns>
    public readonly Matrix4x2D ToMatrix4x2D() => new(M11, M12, M21, M22, M31, M32, M41, M42);

    /// <inheritdoc/>
    public readonly bool Equals(Matrix4x2 other) => this == other;

    /// <inheritdoc/>
    public readonly override bool Equals(object? obj) => obj is Matrix4x2 matrix && this == matrix;
    ...
    public static Matrix4x2 CreateRotation(float angle) => new(Matrix2x2.CreateRotation(angle));

    /// <summary>Creates a scale matrix.</summary>
    /// <param name="scale">The uniform scale factor.</param>
    /// <returns>The created matrix.</returns>
    public static Matrix4x2 CreateScale(float scale) => new(Matrix2x2.CreateScale(scale));

    /// <summary>Creates a scale matrix.</summary>
    /// <param name="xScale">The scale factor of the X axis.</param>
    /// <param name="yScale">The scale factor of the Y axis.</param>
    /// <returns>The created matrix.</returns>
    public static Matrix4x2 CreateScale(float xScale, float yScale) => new(Matrix2x2.CreateScale(xScale, yScale));

    /// <summary>Creates a scale matrix.</summary>
    /// <param name="scale">The scale factor of the X and Y axis.</param>
    /// <returns>The created matrix.</returns>
    public static Matrix4x2 CreateScale(Vector2<float> scale) => new(Matrix2x2.CreateScale(scale));
    ...
    *********/
    ...
    public static Matrix4x2 operator -(Matrix4x2 matrix) => matrix * -1;
    ...
    /// <param name="right">The right operand.</param>
    /// <returns>The result of the multiplication.</returns>
    public static Matrix4x2 operator *(Matrix4x2 left, float right) => right * left;
    ...
    public static Matrix4x2 operator *(Matrix4x2 left, Matrix2x2 right)
    {
        return new(
            ...
            m31: Vector2<float>.Dot(left.Row3, right.Column1),
            m32: Vector2<float>.Dot(left.Row3, right.Column2),
            m41: Vector2<float>.Dot(left.Row4, right.Column1),
            m42: Vector2<float>.Dot(left.Row4, right.Column2)
        );
    }
    ...
    public static Matrix4x3 operator *(Matrix4x2 left, Matrix2x3 right)
    {
        return new(
            ...
            m31: Vector2<float>.Dot(left.Row3, right.Column1),
            m32: Vector2<float>.Dot(left.Row3, right.Column2),
            m33: Vector2<float>.Dot(left.Row3, right.Column3),
            m41: Vector2<float>.Dot(left.Row4, right.Column1),
            m42: Vector2<float>.Dot(left.Row4, right.Column2),
            m43: Vector2<float>.Dot(left.Row4, right.Column3)
        );
    }
    ...
    public static Matrix4x4 operator *(Matrix4x2 left, Matrix2x4 right)
    {
        return new(
            ...
            m32: Vector2<float>.Dot(left.Row3, right.Column2),
            m33: Vector2<float>.Dot(left.Row3, right.Column3),
            m34: Vector2<float>.Dot(left.Row3, right.Column4),
            m41: Vector2<float>.Dot(left.Row4, right.Column1),
            m42: Vector2<float>.Dot(left.Row4, right.Column2),
            m43: Vector2<float>.Dot(left.Row4, right.Column3),
            m44: Vector2<float>.Dot(left.Row4, right.Column4)
        );
    }
    ...
    public static bool operator !=(Matrix4x2 matrix1, Matrix4x2 matrix2) => !(matrix1 == matrix2);

    /// <summary>Converts a matrix to a tuple.</summary>
    /// <param name="matrix">The matrix to convert.</param>
    public static implicit operator (float M11, float M12, float M21, float M22, float M31, float M32, float M41, float M42)(Matrix4x2 matrix) => (matrix.M11, matrix.M12, matrix.M21, matrix.M22, matrix.M31, matrix.M32, matrix.M41, matrix.M42);

    /// <summary>Converts a tuple to a matrix.</summary>
    /// <param name="tuple">The tuple to convert.</param>
    public static implicit operator Matrix4x2((float M11, float M12, float M21, float M22, float M31, float M32, float M41, float M42) tuple) => new(tuple.M11, tuple.M12, tuple.M21, tuple.M22, tuple.M31, tuple.M32, tuple.M41, tuple.M42);

    /// <summary>Converts a <see cref="Matrix4x2"/> to a <see cref="Matrix4x2D"/>.</summary>
    /// <param name="matrix">The matrix to convert.</param>
    public static implicit operator Matrix4x2D(Matrix4x2 matrix) => matrix.ToMatrix4x2D();
}
